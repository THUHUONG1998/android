package com.example.apptracnghiem.question;
...
public class QuestionController {
    ...
    public ArrayList<Question> getQuestion(int num_exam, String subject){
        ...
        Cursor cursor= db.rawQuery("SELECT * FROM tracnghiem1 WHERE num_exam = '"+ num_exam +"' AND subject='"+ subject +"' ORDER BY random()",null);
        cursor.moveToFirst();
        do {
            Question item;
    ...
    public Cursor getQuestionBySubject(String key) {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Cursor cursor=db.rawQuery("SELECT * FROM tracnghiem1 WHERE subject LIKE '%"+key+"%'",null);
        if (cursor != null) {
            cursor.moveToFirst();
        }
        return cursor;
